...
function renderKPI() {
    ...
    var staffList = DB.findAll('staff');
    var html = '<div style="display:flex;justify-content:space-between;margin-bottom:1rem">';
    html += '<h3 style="color:var(--navy);font-size:18px">'+(role === 'staff' ? 'My KPIs & Appraisals' : 'Staff KPI Tracking')+'</h3>';
    if(role === 'admin' || role === 'manager') {
        html += '<button class="btn btn-primary" onclick="showKPIForm()"><i class="ti ti-plus"></i> Add KPI Goal</button>';
    }
    html += '</div>';

    html += '<table><thead><tr><th><span data-i18n="th_staff_name">Staff Name</span></th><th>Goal / Objective</th><th style="width:250px">Score</th><th>Review Period</th><th><span data-i18n="th_attached_file">Attached File</span></th><th>Feedback</th><th><span data-i18n="th_action">Action</span></th></tr></thead><tbody>';
    if(kpis.length === 0) {
